...
async def auto_detect_modules(client: Any, slave_id: int) -> dict[str, int]:
    ...
    for module_type, test_registers in MODULE_TEST_REGISTERS.items():
        ...
        for module_idx in range(max_count):
            ...
            try:
                # Use the client's built-in read method
                if hasattr(client, "read_holding_registers"):
                    result = await client.read_holding_registers(
                        test_register, 1, slave=slave_id
                    )
                else:
                    # Fallback: assume client has async read method
                    result = await client.async_read_holding_registers(
                        test_register, 1, slave_id
                    )
