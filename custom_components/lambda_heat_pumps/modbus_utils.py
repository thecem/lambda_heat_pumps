...
async def async_read_holding_registers(client: AsyncModbusTcpClient, address, count, slave_id=1):
    ...
    try:
        ...
        if api_type == 'slave':
            # pymodbus >= 3.0
            return await client.read_holding_registers(address, count=count, slave=slave_id)
        elif api_type == 'unit':
            # pymodbus 2.x
...
async def async_read_input_registers(client: AsyncModbusTcpClient, address, count, slave_id=1):
    ...
    try:
        ...
        if api_type == 'slave':
            # pymodbus >= 3.0
            return await client.read_input_registers(address, count=count, slave=slave_id)
        elif api_type == 'unit':
            # pymodbus 2.x
